...
/* 	function implode_recur($separator, $arrayvar) {
	...
	function get_all_fields_values( $tag ) {
	  ...
	  function get_user_value( $matches ){
		  // If no record search for a global form record
		if( isset( $GLOBALS['admin_form'] ) ) $form = $GLOBALS['admin_form'];
	...
	function get_post_value( $matches ){
		// If no record search for a global form record
		if( isset( $GLOBALS['admin_form'] ) ) $form = $GLOBALS['admin_form'];
		// If no global record, look for a record stored in the cookie
		if( isset( $form['id'] ) && empty( $form['record'] ) ) $form = fea_instance()->form_display->get_record( $form );
	...
	function get_product_value( $matches ){
		// If no record search for a global form record
		if( isset( $GLOBALS['admin_form'] ) ) $form = $GLOBALS['admin_form'];
		// If no global record, look for a record stored in the cookie
		if( empty( $form['record'] ) ) $form = fea_instance()->form_display->get_record( $form );
	...
	function get_term_value( $matches ){
		// If no record search for a global form record
		if( isset( $GLOBALS['admin_form'] ) ) $form = $GLOBALS['admin_form'];
		// If no global record, look for a record stored in the cookie
		if( empty( $form['record'] ) ) $form = fea_instance()->form_display->get_record( $form );
	  ...
	  function get_sub_field_value( $matches ) {	
		  // If no record search for a global form record
		if( isset( $GLOBALS['admin_form'] ) ) $form = $GLOBALS['admin_form'];
		// If no global record, look for a record stored in the cookie
		if( empty( $form['record'] ) ) $form = fea_instance()->form_display->get_record( $form );
	  ...
	  function get_field_value( $matches ) {	
		// If no record search for a global form record
		if( isset( $GLOBALS['admin_form'] ) ) $form = $GLOBALS['admin_form'];
		// If no global record, look for a record stored in the cookie
		if( empty( $form['record'] ) ) $form = fea_instance()->form_display->get_record( $form );

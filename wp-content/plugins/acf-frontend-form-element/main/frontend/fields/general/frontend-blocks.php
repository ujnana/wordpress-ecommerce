	...
	<div class="values">
			...
			if ( ! empty( $field['value'] ) ) :
				...
				foreach ( $field['value'] as $i => $value ) :
					...
					// validate
					if ( empty( $layouts[ $value['acf_frontend_blocks_layout'] ] ) ) {
						continue;
					}

					// render
					$this->render_layout( $field, $layouts[ $value['acf_frontend_blocks_layout'] ], $i, $value );
...
<div <?php echo acf_esc_attr( $div ); ?>>
			...
			acf_hidden_input(
				array(
					'name'  => $prefix . '[acf_frontend_blocks_layout]',
					'value' => $layout['name'],
				)
			);
			?>
	...
	<?php endif; ?>
		...
		*/
		...
		function load_value( $value, $post_id, $field ) {
			...
			foreach ( $value as $i => $l ) {
				...
				// append to $values
				$rows[ $i ]                  = array();
				$rows[ $i ]['acf_frontend_blocks_layout'] = $l;
		...
		*/
		...
		function format_value( $value, $post_id, $field ) {
			...
			foreach ( array_keys( $value ) as $i ) {
				...
				// get layout name
				$l = $value[ $i ]['acf_frontend_blocks_layout'];
		...
		*/
		...
		function validate_value( $valid, $value, $field, $input ) {
			...
			if ( $count ) {
				...
				foreach ( $value as $i => $row ) {
					...
					// get layout
					$l = $row['acf_frontend_blocks_layout'];
		...
		function update_row( $row, $i, $field, $post_id ) {
			...
			if ( ! is_array( $row ) || ! isset( $row['acf_frontend_blocks_layout'] ) ) {
				return false;
			}

			// get layout
			$layout = $this->get_layout( $row['acf_frontend_blocks_layout'], $field );
		...
		function update_value( $value, $post_id, $field ) {
			...
			if ( ! empty( $value ) ) {
				...
				foreach ( $value as $row ) {
					...
					if ( isset( $old_value[ $i ] ) && $old_value[ $i ] !== $row['acf_frontend_blocks_layout'] ) {
					...
					// append to order
					$new_value[] = $row['acf_frontend_blocks_layout'];
		...
		*/
		...
		public function validate_rest_value( $valid, $value, $field ) {
			...
			$layouts_to_update = array_count_values( array_column( $value, 'acf_frontend_blocks_layout' ) );
		...
		public function get_rest_schema( array $field ) {
			...
			foreach ( $field['layouts'] as $layout ) {
				$layout_schema = array(
					'type'       => 'object',
					'properties' => array(
						'acf_frontend_blocks_layout' => array(
							'type'     => 'string',
							'required' => true,
		...
		public function format_value_for_rest( $value, $post_id, array $field ) {
			...
			foreach ( $value as &$layout ) {
				$name = $layout['acf_frontend_blocks_layout'];
